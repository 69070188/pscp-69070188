...
```text
Input: รับจำนวนเต็มสามจำนวน N (จำนวนคนทั้งหมด), K (ระยะการส่งต่อ), และ T (หมายเลขของขโมย) ในบรรทัดเดียว
Output: จำนวนคนที่ได้พิจารณาตรวจสอบของขวัญ (นับรวมคนที่ 1 และขโมยหากถูกส่งถึงขโมย)
Constraints: การส่งต่อจะนับวนเป็นวงกลมแบบ 1-indexed โดยเหตุการณ์จะจบลงเมื่อของขวัญวนกลับมาที่คนที่ 1 หรือส่งถึงขโมย (คนที่ T)
```
...
```text
Step 1: รับค่า N, K, T แล้วเช็กกรณีพิเศษถ้าขโมยคือคนที่ 1 (T = 1) ให้ตอบ 1 ทันที
Step 2: กำหนดคนปัจจุบันเริ่มที่ 1 และตัวนับคนเป็น 1
Step 3: วนลูปส่งของขวัญไปยังคนถัดไปโดยคำนวณตำแหน่งด้วยสูตร modulo: (current - 1 + k) % n + 1
Step 4: ในแต่ละรอบ เพิ่มตัวนับขึ้น 1 หากคนถัดไปคือคนที่ 1 ให้หยุดลูปทันที (ไม่นับเพิ่ม) แต่ถ้าเป็นขโมย (T) ให้เพิ่มตัวนับแล้วหยุดลูป
Step 5: พิมพ์จำนวนคนที่ได้พิจารณาของขวัญออกมา
ก่อนทำ รู้ว่าจะต้องคำนวณแบบวงกลมด้วย modulo แต่ยังไม่แน่ใจเรื่องขอบเขตเวลา สำหรับ N ขนาดใหญ่สุดที่ 200,000 ว่าการใช้ while True วนทีละรอบจะผ่านหรือไม่
```
...
```text
ตรวจสอบกรณีค่า N และ K น้อยที่สุด เพื่อดูว่าการคำนวน modulo วงกลมขนาดเล็กถูกต้องหรือไม่
```
...
```text
ตรวจสอบกรณีขโมยอยู่ตำแหน่งปลายทางพอดี ก่อนที่จะวนกลับมาที่คนที่ 1 ในรอบถัดไป
```
