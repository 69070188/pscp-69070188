...
```text
โจทย์ให้หาจำนวนกำแพงที่น้อยที่สุดที่ต้องทำลายจากห้อง N ไปห้อง 1
Input: รับจำนวนเต็ม N โดย 1 <= N <= 100000
Output: แสดงจำนวนกำแพงที่ต้องทำลายน้อยที่สุด
Constraints: ถ้าอยู่ห้อง 1 ไม่ต้องทำลายกำแพง ตอบ 0
```
...
```text
รับค่า N
หาว่าห้อง N อยู่ชั้นไหน
หาตำแหน่งของห้องในชั้นนั้น
ตรวจว่าตำแหน่งเป็นเลขคู่หรือคี่
คำนวณจำนวนกำแพงที่ต้องทำลายแล้วแสดงผล
```
...
```text
วิธีสุดท้ายของฉันเหมือนกับแผนแรก โดยใช้ sqrt และ ceil เพื่อหาชั้นของห้อง แล้วใช้ตำแหน่งของห้องในชั้นนั้นตรวจเลขคู่หรือคี่เพื่อคำนวณจำนวนกำแพงขั้นต่ำ
```
...
```text
ทดสอบห้องหมายเลข 1 ซึ่งไม่ต้องทำลายกำแพง
```
...
```text
ทดสอบห้องที่อยู่ในชั้นถัดไปและตำแหน่งเป็นเลขคี่
```
...
```text
ช่วยอธิบายโจทย์ให้่เข้าใจ
```

คุณยังทำอะไรด้วยตนเอง

```text
เขียนโค้ดด้วยตัวเองตามความเข้าใจโจทย์ที่เพื่อนอธิบาย
```
