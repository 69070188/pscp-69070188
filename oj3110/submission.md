...
```text
โจทย์ให้คำนวณค่าธรรมเนียมการส่งพัสดุตามเส้นทางและน้ำหนัก โดยคำนวณจากสูตร: ค่าธรรมเนียมทั้งหมด = ค่าธรรมเนียมเริ่มต้น + (น้ำหนัก × ค่าธรรมเนียมน้ำหนักต่อ กก.) หากเส้นทางไม่อยู่ในตารางอัตราค่าธรรมเนียมให้แสดงผลว่า "Error"
Input: 
บรรทัดที่ 1: รหัสต้นทาง และ รหัสปลายทาง (คั่นด้วยเว้นวรรค)
บรรทัดที่ 2: น้ำหนักพัสดุ (จำนวนจริง)
Output: ค่าส่งพัสดุทศนิยม 2 ตำแหน่ง หรือพิมพ์ "Error" หากเส้นทางไม่ถูกต้อง
Constraints: รับรหัสต้นทาง-ปลายทางเป็นตัวพิมพ์ใหญ่เท่านั้น และแสดงผลราคาเป็นทศนิยม 2 ตำแหน่ง
```
...
```text
Step 1: รับค่าต้นทางและปลายทางมาแยกเป็น 2 ตัวแปรด้วย .split() และรับค่าน้ำหนักเป็น float
Step 2: ตรวจสอบเส้นทางว่าตรงกับ 1 ใน 6 เส้นทางตามตารางหรือไม่ด้วย if-elif
Step 3: หากตรงกับเส้นทางใด ให้คำนวณราคาด้วยสูตร (น้ำหนัก * ค่าธรรมเนียมต่อ กก.) + ค่าธรรมเนียมเริ่มต้น
Step 4: แสดงผลราคาที่คำนวณได้ให้อยู่ในรูปแบบทศนิยม 2 ตำแหน่ง
Step 5: หากไม่ตรงกับเส้นทางใดเลย ให้พิมพ์ "Error"
```
...
```text
ทดสอบการคำนวณเส้นทางสุดท้ายในตารางที่มีอัตราค่าบริการสูงสุด
```
...
```text
ตรวจสอบการจัดการกับเส้นทางที่ไม่ได้อยู่ในเงื่อนไข 6 เส้นทางที่กำหนด
```
