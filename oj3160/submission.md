...
```text
Input: รับจำนวนเต็มสองจำนวนแทนช่วงเริ่มต้น (x) และช่วงสุดท้าย (y) ในบรรทัดเดียว
Output: บรรทัดแรกแสดงรายการจำนวนเฉพาะทั้งหมดในช่วง [x, y] คั่นด้วยช่องว่าง ถ้าไม่มีไม่ต้องแสดง และบรรทัดถัดมาแสดงจำนวนรวมของจำนวนเฉพาะในรูปแบบ "Total primes: [จำนวน]"
Constraints: ต้องตรวจสอบจำนวนเฉพาะให้ถูกต้องตามหลักคณิตศาสตร์ในช่วงตั้งแต่ x ถึง y โดยต้องจัดการกรณีตัวเลขน้อยกว่าหรือเท่ากับ 1 รวมถึงกรณีที่ไม่พบจำนวนเฉพาะเลย
```
...
```text
Step 1: รับค่า x และ y แล้วสร้างลิสต์ว่างสำหรับเก็บจำนวนเฉพาะ
Step 2: วนลูปตั้งแต่ x ถึง y
Step 3: เช็กเงื่อนไขแบบแยกกรณีพิเศษ 2, 3, 5, 7 และลองนำ 2, 3, 5, 7 ไปหารเพื่อกรองตัวเลข
Step 4: ถ้าผ่านเงื่อนไข ให้บันทึกตัวเลขลงในลิสต์
Step 5: พิมพ์จำนวนเฉพาะทั้งหมดที่พบ คั่นด้วยช่องว่าง และพิมพ์จำนวนรวมทั้งหมด
```
...
```text
ตรวจสอบกรณีขอบเขตล่างสุดที่มี 1 และมีจำนวนเฉพาะเพียงตัวเดียวคือ 2
```
...
```text
ตรวจสอบกรณีช่วงที่ไม่มีจำนวนเฉพาะเลย
```
...
```text
ตรวจสอบกรณีที่กำหนดช่วงให้ x และ y เป็นตัวเลขเดียวกัน และตัวเลขนั้นเป็นจำนวนเฉพาะ
```
