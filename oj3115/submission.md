...
```text
โจทย์ต้องการให้หาว่าในนาทีที่กำหนด (จากช่วงเวลา 0 ถึง 1440 นาที) มีร้านค้าเปิดอยู่กี่ร้าน โดยมีร้านค้าทั้งหมด num ร้าน แต่ละร้านจะมีช่วงเวลาเปิด (start) และปิด (stop) ซึ่งร้านจะเปิดในนาทีตั้งแต่ start ถึง stop - 1 (ไม่นับนาทีที่ stop)
Input: 
- บรรทัดที่ 1: จำนวนร้านค้า (num) และ จำนวนเวลาที่ต้องการตรวจสอบ (check)
- บรรทัดที่ 2 ถึง num+1: เวลาเปิด (start) และ เวลาปิด (stop) ของแต่ละร้าน
- บรรทัดสุดท้าย: นาทีที่ต้องการตรวจสอบจำนวน check ค่า คั่นด้วยช่องว่าง
Output: จำนวนร้านที่เปิดอยู่ในแต่ละนาทีที่ตรวจสอบ พิมพ์เรียงกันในบรรทัดเดียวคั่นด้วยช่องว่าง
Constraints: 0 <= start < stop <= 1440 และเวลานับในแต่ละวันไม่เกิน 1440 นาที
```
...
```text
Step 1: รับค่าจำนวนร้าน (amount) และจำนวนครั้งที่ต้องเช็ค (check)
Step 2: สร้างอาร์เรย์/ลิสต์ชื่อ minutes ขนาด 1441 ช่อง กำหนดค่าเริ่มต้นเป็น 0 ทุกช่องเพื่อเก็บจำนวนร้านที่เปิดในนาทีนั้นๆ
Step 3: วนลูปรับช่วงเวลาเปิด-ปิด (start, stop) ของแต่ละร้าน แล้วใช้วงลูปบวกค่าใน minutes[i] เพิ่มขึ้นทีละ 1 สำหรับนาที i ตั้งแต่ start ถึง stop - 1
Step 4: รับรายการนาทีที่ต้องการตรวจสอบ (check_time)
Step 5: ดึงค่าจำนวนร้านที่เปิดจาก minutes[t] ตามเวลา t ที่ระบุ นำมาต่อกันแล้วพิมพ์ออกมาในบรรทัดเดียวคั่นด้วยช่องว่าง
```
...
```text
ทดสอบการนับจำนวนร้านในช่วงเวลาปกติและตรวจสอบการเช็คเวลา stop ที่ต้องไม่นับร้านนั้น
```
...
```text
ตรวจสอบนาทีที่ไม่มีร้านค้าใดเปิดเลย โปรแกรมต้องคืนค่าเป็น 0
```
...
```text
ตรวจสอบขอบของอาร์เรย์นาทีแรกสุด และขอบเขตการทำงานของ minutes
```
