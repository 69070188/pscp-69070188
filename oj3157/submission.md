...
```text
Input: รับจำนวนเต็ม N แทนจำนวนคำสั่ง และรับคำสั่ง "+" หรือ "-" จำนวน N ครั้ง
Output: พิมพ์คะแนนรวมสุดท้าย (เริ่มต้นที่ 0 คะแนน)
Constraints: N อยู่ในช่วง 1 ถึง 1,000 โดยคำสั่ง "+" จะเพิ่ม 10 คะแนน และคำสั่ง "-" จะลด 5 คะแนน คะแนนรวมสามารถติดลบได้
```
...
```text
Step 1: กำหนดตัวแปรคะแนนเริ่มต้นเป็น 0 และรับค่า N
Step 2: ทำการวนลูปรับค่าคำสั่งจำนวน N ครั้ง
Step 3: ในแต่ละรอบ ตรวจสอบคำสั่ง ถ้าเป็น "+" ให้เพิ่มคะแนน 10 ถ้าเป็น "-" ให้ลดคะแนน 5
Step 4: พิมพ์คะแนนรวมสุดท้ายออกมา
ก่อนทำ รู้ว่าต้องใช้การวนลูปร่วมกับ if-elif ตรวจสอบเครื่องหมาย แต่ยังไม่แน่ใจว่าคะแนนจะกลายเป็นค่าติดลบได้หรือไม่ถ้ามีคำสั่งลบเยอะกว่า
```
...
```text
ตรวจสอบการทำงานทั่วไปที่มีทั้งคำสั่งบวกและลบผสมกัน
```
...
```text
ตรวจสอบกรณี N ค่าเล็กที่สุดเพื่อดูว่าลูปและเงื่อนไขทำงานถูกต้องตั้งแต่รอบแรก
```
...
```text
ตรวจสอบกรณีมีแต่คำสั่งลบติดต่อกัน
```
