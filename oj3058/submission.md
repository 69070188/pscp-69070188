...
```text
โจทย์ให้คำนวณจำนวนอิฐก้อนเล็ก 1 นิ้ว ที่น้อยที่สุดเพื่อสร้างสะพานให้ยาว goal นิ้ว โดยต้องใช้อิฐก้อนใหญ่ 5 นิ้ว ให้ได้มากที่สุดก่อน
Input: a (จำนวนอิฐเล็ก), b (จำนวนอิฐใหญ่), goal (ความยาวที่ต้องการ)
Output: จำนวนอิฐเล็กที่ต้องใช้ หากสร้างไม่ได้ให้ตอบ -1
Constraints: ใช้อิฐใหญ่ไม่เกินความต้องการ และใช้อิฐใหญ่ก่อนอิฐเล็กเสมอ
```
...
```text
Step 1: รับค่า a, b, และ goal
Step 2: คำนวณจำนวนอิฐใหญ่ที่ใช้ได้จาก min(b, goal // 5)
Step 3: หาความยาวที่เหลือ remaining_length = goal - (จำนวนอิฐใหญ่ที่ใช้ * 5)
Step 4: ถ้า a >= remaining_length ให้พิมพ์ remaining_length ถ้าไม่พอให้พิมพ์ -1
```
...
```text
เหมือนกับแผนแรก เพราะการใช้อิฐใหญ่ขนาด 5 นิ้วให้มากที่สุดก่อนด้วย min(b, goal // 5) แล้วนำอิฐเล็กมาเติมส่วนที่เหลือ เป็นวิธีคำนวณที่ตรงไปตรงมาและใช้อิฐเล็กน้อยที่สุดแน่นอน
```
...
```text
ทดสอบการทำงานพื้นฐานที่ใช้อิฐทั้งสองชนิดผสมกัน
```
...
```text
ตรวจสอบว่าใช้อิฐใหญ่เท่าที่จำเป็น ไม่ใช้เกิน goal
```
...
```text
ตรวจสอบการคืนค่า -1 เมื่ออิฐทั้งหมดรวมกันไม่ถึงเป้าหมาย
```
